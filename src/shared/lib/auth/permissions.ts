/**
 * Catalogue des permissions granulaires (source unique, utilisée côté serveur
 * pour le gating ET côté client pour les interrupteurs de gouvernance).
 *
 * Règle d'or : un administrateur possède TOUTES les permissions, toujours
 * (non modifiable). Les permissions ne s'appliquent qu'aux comptes commerciaux.
 */
...
export type PermissionDef = {
 ...
 label: string;
 section: string;
 /** Activée par défaut pour un nouveau compte commercial. */
...
/** Sections dans l'ordre d'affichage (dédupliquées). */
...
/** Filtre une liste arbitraire en ne gardant que des clés de permission valides. */
