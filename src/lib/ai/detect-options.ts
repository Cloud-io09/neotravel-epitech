...
const TOLLS = /\bp[ée]ages?\b/iu;
const TOLLS_NEGATED = /\b(?:sans|pas\s+(?:de\s+|d['’]))\s*p[ée]ages?\b/iu;
...
export function detectOptions(message: string): DetectedOptions {
  ...
  if (TOLLS.test(message) && !TOLLS_NEGATED.test(message)) options.tolls = true;
