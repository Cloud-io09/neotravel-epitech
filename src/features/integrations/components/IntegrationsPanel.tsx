...
function IntegrationCard({ integration }: { integration: IntegrationStatus }) {
 ...
 async function onSubmit(event: FormEvent) {
  ...
  if (result.ok) {
   setState({ status: "saved", message: "Enregistre. Redemarrez le serveur pour activer la connexion." });
  } else {
   setState({ status: "error", message: result.error ?? "Echec de l'enregistrement." });
  }
 }
  ...
  <section className={styles.card}>
   ...
   {integration.fields.length > 0 ? (
    <form className={styles.form} onSubmit={onSubmit}>
     <p className={styles.formTitle}>Champs necessaires pour connecter</p>
     <div className={styles.grid}>
      {integration.fields.map((field) => (
       <label key={field.key} className={styles.field}>
        <span className={styles.fieldLabel}>
         {field.label} <code>{field.key}</code>
         {field.isSet ? <em className={styles.fieldSet}>defini</em> : null}
        </span>
        <input
         type={field.secret ? "password" : "text"}
         autoComplete="off"
         placeholder={
          field.isSet ? "******** (deja defini - laisser vide pour conserver)" : field.placeholder ?? ""
         }
 ...
 return (
  <main className={dashStyles.page}>
   <header className={dashStyles.header}>
    <div>
     <p className={dashStyles.eyebrow}>Systeme</p>
     <h1>Connexions</h1>
     <p>
      Etat des integrations - <strong>{connected}/{integrations.length} connectees</strong>. Remplissez les champs
      d&apos;une integration non active puis redemarrez le serveur pour l&apos;activer.
     </p>
    </div>
   </header>
